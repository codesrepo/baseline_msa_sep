    ...
    def __init__(self, opt, shared=None):
        ...
        if shared:
            ...
            if opt.get('dict_tokenizer') == 'bpe' and opt['embedding_type'] != 'random':
                ...
                enc_wgts = np.load(PRE_LM_PATH)
                row_m = enc_wgts.mean(0)
                row_glove = embs.vectors.mean(0)
                with open(PRE_PATH, 'rb') as f:
                    itos2 = pickle.load(f)
                stoi2 = collections.defaultdict(lambda:-1, {v:k for k,v in enumerate(itos2)})
                for w, i in self.dict.tok2ind.items():
                    if w in embs.stoi:
                         vec_glove = embs.vectors[embs.stoi[w]]
                         #self.model.decoder.lt.weight.data[i] = vec
                    else:
                        vec_glove = row_glove
                    r = stoi2[w]
                    if r > 0:
                        self.model.decoder.lt.weight.data[i] = torch.tensor(np.concatenate((enc_wgts[r],vec_glove),axis=0))
                        self.model.encoder.lt.weight.data[i] = torch.tensor(np.concatenate((enc_wgts[r],vec_glove),axis=0))
                        cnt += 1
                    else:
                        self.model.decoder.lt.weight.data[i] =  torch.tensor(np.concatenate((row_m,vec_glove),axis=0))
                        self.model.encoder.lt.weight.data[i] =  torch.tensor(np.concatenate((row_m,vec_glove),axis=0))                    
                    # if w in embs.stoi:
