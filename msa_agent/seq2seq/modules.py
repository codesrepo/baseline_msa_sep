...
class Seq2seq(nn.Module):
    ...
    def __init__(self, opt, num_features,
        ...
        self.esz = opt['embeddingsize']

        self.NULL_IDX = padding_idx
        self.END_IDX = end_idx
        self.register_buffer('START', torch.LongTensor([start_idx]))
        self.longest_label = longest_label
        self.highway = Highway(1024, 1, f=torch.nn.functional.sigmoid)
...
class Encoder(nn.Module):
    def __init__(self, num_features, padding_idx=0, rnn_class='lstm',
        ...
        self.highway = Highway(hidden_size, 1, f=torch.nn.functional.sigmoid)
    ...
    def forward(self, xes, hidden, enc_out, attn_mask=None):
        ...
        if self.attention == 'local':
            ...
            if self.attention == 'concat':
                ...
                hid = self.attn(hid)
                #print("hidden, enc_out,xes,xorig size", hid.size(),enc_out.transpose(1, 2).size(),xraw.squeeze(1).size(), xorig.transpose(1, 2).size())
                #torch.bmm(xraw.view(len(xraw),1,self.esz), xorig.transpose(1, 2)).squeeze(1)
